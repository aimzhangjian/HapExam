...
public class HapOrgCompanysController  extends BaseController{
    ...
    @Autowired
    private HapOrgCompanysService hapOrgCompanysService;

    @RequestMapping("/hapOrgCompanys/query")
    @ResponseBody
    public ResponseData selectList(HttpServletRequest request, HapOrgCompanys condition,
                                   @RequestParam(defaultValue = DEFAULT_PAGE) int page,
                                   @RequestParam(defaultValue = DEFAULT_PAGE_SIZE) int pagesize) {
        IRequest iRequest = createRequestContext(request);
        List<HapOrgCompanys> datas = hapOrgCompanysService.selectHapOrgCompanysList(iRequest, condition, page, pagesize);
