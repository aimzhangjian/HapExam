...
public class HapArCustomersController  extends BaseController{
    ...
    @Autowired
    private HapArCustomersService hapArCustomersService;

    @RequestMapping("/HapArCustomers/query")
    @ResponseBody
    public ResponseData selectList(HttpServletRequest request, HapArCustomers condition,
                                   @RequestParam(defaultValue = DEFAULT_PAGE) int page,
                                   @RequestParam(defaultValue = DEFAULT_PAGE_SIZE) int pagesize) {
        IRequest iRequest = createRequestContext(request);
        List<HapArCustomers> datas = hapArCustomersService.selectHapArCustomersList(iRequest, condition, page, pagesize);
