...
public class HapInvInventoryItemsController  extends BaseController{
    ...
    @Autowired
    private HapInvInventoryItemsService hapInvInventoryItemsService;

    @RequestMapping("/hapInvInventoryItems/query")
    @ResponseBody
    public ResponseData selectList(HttpServletRequest request, HapInvInventoryItems condition,
                                   @RequestParam(defaultValue = DEFAULT_PAGE) int page,
                                   @RequestParam(defaultValue = DEFAULT_PAGE_SIZE) int pagesize) {
        IRequest iRequest = createRequestContext(request);
        List<HapInvInventoryItems> datas = hapInvInventoryItemsService.selectHapInvInventoryItemsList(iRequest, condition, page, pagesize);
