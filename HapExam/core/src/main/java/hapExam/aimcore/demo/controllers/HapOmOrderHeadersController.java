...
public class HapOmOrderHeadersController  extends BaseController{
    ...
    @Autowired
    private HapOmOrderHeadersService hapOmOrderHeadersService;

    @RequestMapping("/hapOmOrderHeaders/query")
    @ResponseBody
    public ResponseData selectList(HttpServletRequest request, HapOmOrderHeaders condition,
                                   @RequestParam(defaultValue = DEFAULT_PAGE) int page,
                                   @RequestParam(defaultValue = DEFAULT_PAGE_SIZE) int pagesize) {
        IRequest iRequest = createRequestContext(request);
        List<HapOmOrderHeaders> datas = hapOmOrderHeadersService.selectHapOmOrderHeadersList(iRequest, condition, page, pagesize);
